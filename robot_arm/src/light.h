...
public:
	...
	void updateLightDir() {
		...
		// Direction from light to surface (sun to earth)
		lightDir.x = cos(el_rad) * sin(az_rad);
		lightDir.y = -sin(el_rad);
		lightDir.z = cos(el_rad) * cos(az_rad);
	...
	void processKeyboard(float xoffset, float yoffset)
	{
		...
		this->elevation = MIN(this->elevation, 80.0f);
		this->elevation = MAX(this->elevation, 15.0f);
		updateLightDir();
	}
};
